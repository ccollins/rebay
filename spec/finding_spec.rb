...
module Rebay
  describe Finding do
    ...
    context "after creation" do
      ...
      it "should provide find_items_by_category" do
        @finder.should respond_to(:find_items_by_image)
      end

      it "should provide find_items_by_category" do
        @finder.should respond_to(:find_items_by_keywords)
      end
    ...
    context "when calling find_items_by_image" do
      it "should fail without args" do
        lambda { @finder.find_items_by_image }.should raise_error(ArgumentError)
      end

      it "should return a hash response" do
        @finder.find_items_by_image({:itemId => 1}).class.should eq(Rebay::Response)
      end

      it "should succeed" do
        @finder.find_items_by_image({:itemId => 1}).success?.should be_true
      end

      it "should iterate over results" do
        json = JSON.parse(File.read(File.dirname(__FILE__) + "/json_responses/finding/find_items_by_image"))
        @finder.stub!(:get_json_response).and_return(Rebay::Response.new(json))
        response = @finder.find_items_by_image({:itemId => 1})
